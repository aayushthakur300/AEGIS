...
// ✅ FIX 1: Removed Duplicate "import io..."

// Automatically detects if you are on localhost or https://aegis.onrender.com
const socket = io(window.location.origin, {
    path: '/socket.io',
    transports: ['websocket', 'polling'] // Force stable connection
});
...
export default function Dashboard() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      // ✅ FIX 2: Changed http://localhost:5000/api/analyze to /api/analyze
  ...
  const handleChat = async () => {
      ...
      try {
        // ✅ FIX 3: Changed http://localhost:5000/api/chat to /api/chat
        const res = await fetch("/api/chat", { 
            method: "POST", 
            headers: {"Content-Type":"application/json"}, 
            body: JSON.stringify({ message: msg.text, context: analysis?.rootCause || "No active analysis" }) 
        });
        const data = await res.json();
